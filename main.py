        ...
        async with httpx.AsyncClient() as client:
            r = await client.get(
                ...
                params={
                    ...
                    "select": "seconds_used,minutes_used,month_year"
                },
                headers=supabase_headers(),
                timeout=10
            )
            print(f"Get usage: {r.status_code} - {r.text}")
            ...
            if r.status_code == 200:
                ...
                if data and len(data) > 0:
                    ...
                    if "minutes_used" in row and row["minutes_used"]:
                        return float(row["minutes_used"])
                    elif "seconds_used" in row and row["seconds_used"]:
                        return float(row["seconds_used"]) / 60.0
        return 0.0
    except Exception as e:
        print(f"Get usage error: {e}")
        return 0.0

async def add_usage(user_id: str, minutes: float) -> float:
    """Add minutes to user usage - handles both seconds_used and minutes_used columns"""
    try:
        now = datetime.utcnow()
        month_year = f"{now.year}-{now.month:02d}"
        seconds = minutes * 60
        ...
        async with httpx.AsyncClient() as client:
            # Check existing row for this month
            r = await client.get(
                f"{SUPABASE_URL}/rest/v1/usage",
                params={
                    "user_id": f"eq.{user_id}",
                    "month_year": f"eq.{month_year}",
                    "select": "id,seconds_used,minutes_used"
                },
                headers=supabase_headers(),
                timeout=10
            )
            print(f"Usage check: {r.status_code} - {r.text}")
            existing = r.json() if r.status_code == 200 else []
            
            if existing and len(existing) > 0:
                row = existing[0]
                row_id = row["id"]
                cur_min = float(row.get("minutes_used") or 0)
                cur_sec = float(row.get("seconds_used") or 0)
                # Use whichever column has data
                if cur_min > 0:
                    new_min = cur_min + minutes
                    new_sec = new_min * 60
                else:
                    new_sec = cur_sec + seconds
                    new_min = new_sec / 60
                
                upd = await client.patch(
                    f"{SUPABASE_URL}/rest/v1/usage",
                    params={"id": f"eq.{row_id}"},
                    json={"minutes_used": new_min, "seconds_used": new_sec, "updated_at": now.isoformat()},
                    headers=supabase_headers(),
                    timeout=10
                )
                print(f"Usage updated: {upd.status_code} - new_min={new_min:.2f}")
                return new_min
            else:
                ...
                print(f"Usage inserted: {ins.status_code} - {ins.text}")
                return minutes
    except Exception as e:
